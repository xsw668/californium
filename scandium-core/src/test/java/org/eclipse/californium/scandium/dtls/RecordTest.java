...
public class RecordTest {
	...
	public void testConstructorEnforcesMaxSequenceNo() throws GeneralSecurityException {
		new Record(ContentType.HANDSHAKE, 0, DtlsTestTools.MAX_SEQUENCE_NO, new HelloRequest(session.getPeer()), session, true);
		try {
			new Record(ContentType.HANDSHAKE, 0, DtlsTestTools.MAX_SEQUENCE_NO + 1, new HelloRequest(session.getPeer()), session, true);
