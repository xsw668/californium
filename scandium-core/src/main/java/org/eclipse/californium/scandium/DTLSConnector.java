...
public class DTLSConnector implements Connector, RecordLayer {
	...
	private final boolean useConnectionIdInHandshake;
	...
	protected DTLSConnector(final DtlsConnectorConfig configuration, final ResumptionSupportingConnectionStore connectionStore) {
		if (configuration == null) {
			...
			this.useConnectionIdInHandshake = config.isConnectionIdUsedInHandshake();
	...
	public final void close(InetSocketAddress peerAddress) {
		final Connection connection = getAliveConnection(peerAddress, null, false);
	...
	private void start(final InetSocketAddress bindAddress) throws IOException {
		...
		if (executorService instanceof ScheduledExecutorService) {
			...
			timer = ExecutorsUtil.newSingleThreadScheduledExecutor(
					new DaemonThreadFactory("DTLS-Retransmit-Task-" + tag, NamedThreadFactory.SCANDIUM_THREAD_GROUP)); //$NON-NLS-1$
		}
		...
		if (executorService == null) {
			...
			if (threadCount > 1) {
				executorService = ExecutorsUtil.newFixedThreadPool(threadCount - 1,
						new DaemonThreadFactory("DTLS-Connection-Handler-" + tag, NamedThreadFactory.SCANDIUM_THREAD_GROUP)); //$NON-NLS-1$
			} else {
		...
		for (int i = 0; i < receiverThreadCount; i++) {
			Worker receiver = new Worker("DTLS-Receiver-" + tag + "." + i + "-" + lastBindAddress) {
	...
	/**
	 * Get connection with serial executor for peer.
	 * 
	 * @param peerAddress socket address of peer
	 * @param create {@code true}, create new connection, if connection is not
	 *            available.
	 * @return connection with executing serial executor. {@code null}, if store
	 *         is exhausted or if the connection is not available and create is
	 *         {@code false}.
	 */
	private final Connection getAliveConnection(InetSocketAddress peerAddress, ConnectionId cid, boolean create) {
		...
		synchronized (connectionStore) {
			...
			if (connection == null) {
				if (cid == null && create) {
					LOGGER.debug("{}create new connection for {}", tag, peerAddress);
					newConnection = new Connection(peerAddress, new SerialExecutor(executor));
				} else {
					LOGGER.debug("{}no connection available for {},{}", tag, peerAddress, cid);
				}
			} else if (!connection.isExecuting() && running.get()) {
				LOGGER.debug("{}recreate new connection for {}", tag, connection);
			...
			if (newConnection != null) {
				...
				LOGGER.trace("{}connection available for {},{}", tag, peerAddress, cid);
			}
			return connection;
		}
	}
	...
	/**
	 * Get initial connection with serial executor and without handshaker nor session.
	 * 
	 * @param connection connection
	 * @return connection initial connection.
	 */
	private final Connection getInitialConnection(Connection connection, boolean removeFromSessionCache) {
		boolean stopped = true;
		if (connection.hasEstablishedSession() || connection.hasOngoingHandshake() || !connection.isExecuting()) {
			SerialExecutor executor;
			if (connection.isExecuting()) {
				executor = connection.getExecutor();
			} else {
				executor = new SerialExecutor(getExecutorService());
			}
		...
		if (stopped) {
			terminateOngoingHandshake(connection, new IllegalStateException("connector stopped!"),
					AlertDescription.INTERNAL_ERROR);
			return null;
		} else {
			terminateOngoingHandshake(connection, new IllegalStateException("connection store exhausted!"),
					AlertDescription.INTERNAL_ERROR);
			return null;
		}
	}
	...
	private void receiveNextDatagramFromNetwork(DatagramPacket packet) throws IOException {
		...
		ConnectionId connectionId = fristRecord.getConnectionId();
		connection = getAliveConnection(peerAddress, connectionId, false);
		if (connection == null) {
			if (records.size() == 1 && fristRecord.isNewClientHello()) {
				...
				LOGGER.debug("{}Discarding {} records from [{}] received without existing connection", tag,
	...
	private void processRecord(Record record, Connection connection) {
		...
		try {
			...
			if (record.getType() == ContentType.TLS12_CID) {
				...
				if (!useConnectionIdInHandshake && epoch == 0) {
	...
	private void processNewClientHello(final Record record) {
		...
		try {
			...
			if (isClientInControlOfSourceIpAddress(clientHello, record, null)) {
				...
				final Connection connection = getAliveConnection(peerAddress, null, true);
				if (connection == null) {
					return;
				}
				SerialExecutor serialExecutor = connection.getExecutor();
	...
	private void processApplicationDataRecord(final Record record, final Connection connection) {
		...
		if (session != null) {
			...
			if (session.isRecordProcessable(record.getEpoch(), record.getSequenceNumber())) {
				try {
					...
					if (ongoingHandshake != null) {
						// the handshake seems to have been completed successfully
					...
					LOGGER.error("{}discarding APPLICATION_DATA record received from peer [{}]", tag,
							record.getPeerAddress(), e);
					discardRecord(record, e);
				}
			} else {
				LOGGER.error("{}discarding duplicate APPLICATION_DATA record received from peer [{}]", tag,
						record.getPeerAddress());
			}
		} else {
			if (ongoingHandshake != null && ongoingHandshake.isChangeCipherSpecMessageExpected()) {
				...
				LOGGER.error("{}discarding APPLICATION_DATA record received from peer [{}] without an active session",
						tag, record.getPeerAddress());
			}
		}
	}
	 ...
	 * <code>generateCookie</code> method.
	 * </p>
	 * <p>
	 * This method sends a <em>HELLO_VERIFY_REQUEST</em> to the peer, if the
	 * cookie contained in <code>clientHello</code> does not match the expected
	 * cookie.
	 * </p>
	...
	private void startNewHandshake(final ClientHello clientHello, final Record record, final Connection connection) throws HandshakeException {
		final Connection peerConnection = getInitialConnection(connection, true);
		if (peerConnection != null) {
			if (peerConnection.isExecuting()) {
	...
	void send(AlertMessage alert, DTLSSession session) {
		if (alert == null) {
			...
			try {
				sendRecord(new Record(ContentType.ALERT, session.getWriteEpoch(), session.getSequenceNumber(), alert, session, true));
			} catch (IOException e) {
				// already logged ...
			} catch (GeneralSecurityException e) {
	...
	public final void send(final RawData msg) {
		...
		if (!running.get()) {
			...
			connection = getAliveConnection(msg.getInetSocketAddress(), null, true);
			if (connection == null) {
		...
		if (pendingOutboundMessagesCountdown.decrementAndGet() >= 0) {
			try {
				final Connection sendConnection = connection;
				if (connection.getExecutor() == null) {
					throw new NullPointerException("missing executor for connection! " + connection.getSessionTicket());
				}
				sendConnection.getExecutor().execute(new Runnable() {

					@Override
					public void run() {
						try {
							if (running.get()) {
								sendMessage(msg, sendConnection);
							} else {
	...
	private boolean checkOutboundEndpointContext(final RawData message, final Connection connection,
		...
		if (null != endpointMatcher && !endpointMatcher.isToBeSent(message.getEndpointContext(), connectionContext)) {
			LOGGER.warn("{}DTLSConnector ({}) drops {} bytes for {} to {} != {}", tag, this, message.getSize(),
	...
	private void sendFlightOverNetwork(DTLSFlight flight) throws IOException {
		...
		if (flight.getSession() != null) {
			...
			cid = flight.getSession().getWriteConnectionId();
		}
		...
		for (Record record : flight.getMessages()) {
			if (useConnectionIdInHandshake && payload.length == 0 && cid != null) {
				LOGGER.info("add [{}], type {}, {} bytes", cid, record.getType(), record.getLength());
				record.setConnectionId(cid);
			}
